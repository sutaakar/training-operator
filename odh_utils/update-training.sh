...
if [ "$DETECTED_PATTERN" = "rhods-operator" ]; then
    ...
    # Fallback to operator namespace if pattern doesn't match
    DEPLOYMENT_NAMESPACE="$OPERATOR_NAMESPACE"
fi
...
OPERATOR_POD_NAME=$(oc get pods -n "$DEPLOYMENT_NAMESPACE" -l "name=${OPERATOR_DEPLOYMENT_NAME}" -o jsonpath='{.items[0].metadata.name}')

if [ -z "$OPERATOR_POD_NAME" ]; then
    echo "❌ Error: Could not find operator pod in namespace '$DEPLOYMENT_NAMESPACE' with label 'name=${OPERATOR_DEPLOYMENT_NAME}'."
    echo "   Trying alternative label selector..."
    OPERATOR_POD_NAME=$(oc get pods -n "$DEPLOYMENT_NAMESPACE" --selector="app=${OPERATOR_DEPLOYMENT_NAME}" -o jsonpath='{.items[0].metadata.name}')
